...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "Scale", "CenterCrop", "Pad", "PadToSize",
...
class ToPILImage(object):
    ...
    def __call__(self, pic, lbl):
        ...
        """
        return F.to_pil_image(pic, self.mode), F.to_pil_image(lbl, self.mode)
...
class FiveCrop(object):
    ...
    def __call__(self, img, lbl):
        return F.five_crop(img, self.size), F.five_crop(lbl, self.size)
...
class TenCrop(object):
    ...
    def __call__(self, img, lbl):
        return (F.ten_crop(img, self.size, self.vertical_flip),
               F.ten_crop(lbl, self.size, self.vertical_flip))
...
class LinearTransformation(object):
    ...
    def __call__(self, tensor, lbl):
        ...
        flat_lbl = lbl.view(1, -1)
        transformed_lbl = torch.mm(flat_lbl, self.transformation_matrix)
        lbl = transformed_lbl.view(lbl.size())
        return tensor, lbl
...
class Grayscale(object):
    ...
    def __call__(self, img, lbl):
        ...
        """
        return F.to_grayscale(img, num_output_channels=self.num_output_channels), lbl
...
class RandomGrayscale(object):
    ...
    def __call__(self, img, lbl):
        ...
        if random.random() < self.p:
            return F.to_grayscale(img, num_output_channels=num_output_channels), lbl
        return img, lbl
